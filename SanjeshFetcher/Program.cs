...
namespace SanjeshFetcher
{
    class Program
    {
        ...
        private static Student GetStudentData(string parvande, string rahgiri, string idCode)
        {
            ...
            {
                Parvande = parvande,
                Type = StudentType.Unknown
            };
            var doc = new HtmlDocument();
            doc.LoadHtml(content);
            // Haha! string contains goes brrrrrr
            if (content.Contains("رياضي فيزيك"))
                student.Type = StudentType.Math;
            else if (content.Contains("علوم تجربي"))
                student.Type = StudentType.Tajrobi;
            else if (content.Contains("ادبيات وعلوم انساني"))
                student.Type = StudentType.Ensani;
        ...
        /// <returns></returns>
        private static string TypeToString(StudentType type)
        {
            switch (type)
            {
                case StudentType.Math:
                    return "ریاضی";
                case StudentType.Tajrobi:
                    return "تجربی";
                case StudentType.Ensani:
                    return "انسانی";
                default:
                    return "";
            }
